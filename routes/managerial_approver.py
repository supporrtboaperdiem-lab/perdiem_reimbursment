from flask import Blueprint, Response, flash, jsonify, render_template, request, redirect, url_for, abort
...
managerial_approver_bp = Blueprint("managerial_approver", __name__, url_prefix="/managerial_approver")


@managerial_approver_bp.route("/<int:request_id>", methods=["GET", "POST"])
@login_required
def managerial_approver_dashboard(request_id):
    ...
    if request.method == "POST":
        ...
        if is_decline:
            perdiem.status = "DECLINED_BY_MANAGERIAL_APPROVER"
            perdiem.status_final_apr = "DECLINED"
            flash("Request Declined","danger")
        else:
            perdiem.status = "APPROVED_BY_MANAGERIAL_APPROVER"
    ...
    return render_template(
        "managerial_approver_dashboard.html",
        perdiem=perdiem,
...
# -----------------------
@login_required
@managerial_approver_bp.route("/delegate", methods=["POST"])
...
@login_required
@managerial_approver_bp.route("/accept_delegation", methods=["POST"])
...
@login_required
@managerial_approver_bp.route("/decline_delegation", methods=["POST"])
...
@managerial_approver_bp.route('/terminate_delegation', methods=['POST'])
@login_required
...
@managerial_approver_bp.route("/serve_pdf/<int:request_id>/<string:field>")
@login_required
...
@managerial_approver_bp.route("/serve_receipt/<int:receipt_id>")
@login_required
